...
export const updateWeather = function (lat, lon) {
  ...
  fetchData(url.currentWeather(lat, lon), function (currentWeather) {
    ...
    //TODAY'S HIGHLIGHTS
    highlightSection.innerHTML = `
        <h2 class="title-2">What's Happening Now?</h2>
    ...
    fetchData(url.reverseGeo(lat, lon), function ([{ name, country }]) {
      ...
      // Create a counter to keep track of the current result index
      let resultIndex = 0;

      // Function to update the result paragraph with the next result
      function updateResult() {
        if (resultIndex < 7) {
          resultParagraph.textContent = text[resultIndex];
          resultIndex++;
        } else {
          // If all results have been shown, reset the index to 0
          resultIndex = 0;
          resultParagraph.textContent = text[resultIndex];
        }
      }
      ...
      // Attach a click event listener to the "Regenerate!" button
      button_1.addEventListener('click', updateResult);
